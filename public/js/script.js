...
const editFaculty = (facultyId, facultyCode, facultyName, deanCode) => {
    ...
    let action = `/administrator/master/faculties/${facultyId}`;
...
const addWeight = () => {
    let blockTitle = document.querySelector("#dm-add-server h3.block-title");
    blockTitle.textContent = 'Tambah Data';
    // let action = `/administrator/master/weight/list`;

    let formWeight = document.querySelector("#dm-add-server form");
    formWeight.setAttribute("method", "POST");

    Dashmix.block('open', '#dm-add-server');
}

const editWeight = (dataId, weightId, weightReg, weightGross, weightTare, weightNet) => {
    let blockTitle = document.querySelector("#dm-add-server h3.block-title");
    blockTitle.textContent = 'Edit Data';
    let action = `/administrator/master/weight/list/${dataId}/${weightId}`;

    let formWeight = document.querySelector("#dm-add-server form");
    formWeight.setAttribute("method", "POST");
    // formWeight.setAttribute("action", action);

    let inputWeightReg = document.querySelector("#dm-add-server input[name=no_reg]");
    let inputWeightGross = document.querySelector("#dm-add-server input[name=gross]");
    let inputWeightTare = document.querySelector("#dm-add-server input[name=tare]");
    let inputWeightNet = document.querySelector("#dm-add-server input[name=net]");
    let inputMethod = document.querySelector("#dm-add-server input[name=_method]");

    inputWeightReg.value = weightReg;
    inputWeightGross.value = weightGross;
    inputWeightTare.value = weightTare;
    inputWeightNet.value = weightNet;
    inputMethod.value = "PUT";
...
const addStudyProgram = () => {
    ...
    formStudyProgram.setAttribute('action', '/administrator/master/study-program');
...
const editStudyProgram = (studyProgramId, studyProgramCode, name, level, facultyCode, lecturerCode) => {
    const blockTitle = document.querySelector("#dm-add-server h3.block-title");
    blockTitle.textContent = 'Edit Data';
    const action = `/administrator/master/study-programs/${studyProgramId}`;
...
const editScienceField = (scienceFieldId, scienceFieldCode, scienceFieldName) => {
    let blockTitle = document.querySelector("#dm-add-server h3.block-title");
    blockTitle.textContent = 'Edit Data';
    let action = `/administrator/master/science-fields/${scienceFieldId}`;
...
const addThesisRequirement = () => {
    ...
    formThesisRequirement.setAttribute("action", '/administrator/thesis-requirements');
...
const editThesisRequirement = (thesisRequirementId, documentName, documentType, isRequired, note) => {
    let blockTitle = document.querySelector("#dm-add-server h3.block-title");
    blockTitle.textContent = 'Edit Data';
    let action = `/administrator/thesis-requirements/${thesisRequirementId}`;
...
const addAssessmentComponent = () => {
    ...
    formAssessmentComponent.setAttribute("action", '/administrator/assessment-components');
...
const editAssessmentComponent = (assessmentComponentId, name, assessmentType, weight) => {
    let blockTitle = document.querySelector("#dm-add-server h3.block-title");
    blockTitle.textContent = 'Edit Data';
    let action = `/administrator/assessment-components/${assessmentComponentId}`;
