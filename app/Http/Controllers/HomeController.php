...
class HomeController extends Controller
{
    ...
    public function index()
    {
        ...
        //Academic Staff
        if (auth()->user()->level === User::ADMINISTRATOR) {
            $lecturerCount = Lecturer::count();
            $studentCount = Student::count();
            $studyProgramCount = StudyProgram::count();
            $userCount = User::where('level', 'ADMINISTRATOR')->count();

            return viewAdministrator('dashboard', compact('lecturerCount', 'studentCount', 'studyProgramCount', 'userCount'));
        }

        if (auth()->user()->level === User::DRIVER) {
            $lecturerCount = Lecturer::count();
            $studentCount = Student::count();
            $studyProgramCount = StudyProgram::count();
            $userCount = User::where('level', 'DRIVER')->count();

            return viewDriver('dashboard', compact('lecturerCount', 'studentCount', 'studyProgramCount', 'userCount'));
        }
